...
processes = []
result = {'classification': 'invalid', 'exit_code': 125}
...
try:
    ...
    driver = start(['/usr/bin/python3', str(base / 'ime-driver.py'), 'hello'],
                   'driver.log', env)
    ...
    if not state['sent'] or not state['active'] or 'PREEDIT: hello' not in probe_log:
        raise RuntimeError('The expected preedit was not observed')
    if state['committed'] and not state['reset'] and state['text'] == 'hello ' and 'BUFFER: hello ' in probe_log:
        result.update(classification='good', exit_code=0)
    elif state['reset'] and not state['committed'] and state['text'] == '' and 'BUFFER:' not in probe_log:
