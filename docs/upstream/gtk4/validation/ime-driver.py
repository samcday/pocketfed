...
def surrounding(data,obj,text,cursor,anchor): state['text']=(text or b'').decode(); state['cursor']=cursor; state['anchor']=anchor
@callback(None,P,P,U)
...
def done(*args):
 state['serial']+=1
 state['last_done_at']=time.monotonic()
 events.append({'serial':state['serial'],'cause':state['cause'],'text':state['text'],'cursor':state.get('cursor'),'anchor':state.get('anchor')})
...
while time.monotonic()-start < 2:
 if state['active'] and not state['sent'] and state['serial'] and time.monotonic()-state['last_done_at']>0.1:
  state['sent']=True; state['sent_at']=time.monotonic()
  data=word.encode(); send(1,S(data),I(len(data)),I(len(data))); send(3,U(state['serial']))
 if state['active'] and state['sent'] and not state['reset'] and not state['committed'] and time.monotonic()-state['sent_at']>0.6:
