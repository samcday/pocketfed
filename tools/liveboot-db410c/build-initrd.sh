...
Required:
  ...
  --kernel-bundle <dir>    Directory with Image.gz, the selected device DTB and
...
Common options:
  --device <name>          db410c (default) or samsung-a5u-eur.
  ...
  --run-token <token>      pocketfed.liveboot= token and output filename stem
                           (default $RUN_TOKEN).
  --edid-override <path>   drm.edid_firmware override (default 720p on DB410c,
                           none on A5; generated by gen-edid.py for DB410c);
...
case "$DEVICE" in
    db410c)
        DTB_REL=qcom/apq8016-sbc.dtb
        [ "$EDID_OVERRIDE" != auto ] || EDID_OVERRIDE=edid/1280x720.bin
        VIDEO_ARGS=(video=HDMI-A-1:1280x720@60e)
        ;;
    samsung-a5u-eur)
        DTB_REL=qcom/msm8916-samsung-a5u-eur.dtb
        [ "$EDID_OVERRIDE" != auto ] || EDID_OVERRIDE=none
        [ "$EDID_OVERRIDE" = none ] || die 'A5 does not use an HDMI EDID override'
        ADD_DRIVERS='qcom_hwspinlock qcom_apcs_ipc_mailbox qcom_smd rpm_proc smd_rpm clk_smd_rpm qnoc_msm8916 icc_smd_rpm qcom_spmi_regulator qcom_smd_regulator rtc_pm8xxx ulpi phy_qcom_usb_hs ci_hdrc ci_hdrc_msm extcon_sm5502 i2c_gpio i2c_qup gpio_keys pm8941_pwrkey ublk_drv brd libcomposite usb_f_fs msm panel_samsung_ea8061v_ams497ee01 mms114 tm2_touchkey rt5033 rt5033_regulator'
        DISPLAY_ROOTS='msm panel_samsung_ea8061v_ams497ee01'
        VIDEO_ARGS=(oops=panic)
        ;;
...
[ -f "$KERNEL_BUNDLE/$DTB_REL" ] || die "missing $KERNEL_BUNDLE/$DTB_REL"
...
if [ -n "$EDID_OVERRIDE" ]; then
    # Only the default 1280x720 block is generated; another --edid-override
    # path must already exist in the served root's firmware directory.
    mkdir -p "$STAGING/95pocketfed-liveboot/edid"
    python3 "$SELF_DIR/gen-edid.py" "$STAGING/95pocketfed-liveboot/edid/1280x720.bin"
fi
...
# Pocketboot's boot-image loader passes only the 512-byte cmdline field and
# ignores extra_cmdline, so anything past 511 characters is silently dropped;
# keep the line short and refuse to build one that would be truncated.
...
_parts=(
    ...
    rd.emergency=reboot enforcing=0
    "${VIDEO_ARGS[@]}" panic=10
...
CMD_LINE="${_parts[*]}"
[ "${#CMD_LINE}" -le 511 ] \
    || die "kernel command line is ${#CMD_LINE} characters; Pocketboot truncates at 511"
...
cmp "$OUT_DIR/unpacked/dtb" "$KERNEL_BUNDLE/$DTB_REL"
