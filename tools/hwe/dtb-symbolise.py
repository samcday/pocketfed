...
def compile_include_paths(kernel_tree: Path) -> list[str]:
    # The source tree may be a partial checkout: only dt-bindings and the qcom
    # devicetree directory are needed to compile the board DTS. Skip any of the
    # usual include roots that are not present.
    candidates = [
        kernel_tree / "scripts/dtc/include-prefixes",
        kernel_tree / "include",
        kernel_tree / "arch/arm64/boot/dts",
    ]
...
def symbolise(args: argparse.Namespace) -> Path:
    ...
    try:
        ...
        report = {
            ...
            "kernel_source": {"tree": str(kernel_tree), "tag": args.source_tag},
