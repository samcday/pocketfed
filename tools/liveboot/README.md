...
Select the designated test device and its independently identified UART. Set
`TEST_DEVICE_SERIAL` to that phone's exact fastboot serial and `TEST_DEVICE_UART`
to its adapter's path under `/dev/serial/by-id/`. Inspect current ownership; do
not assume an adapter or phone is free because an old run ended.

```sh
run=out/liveboot/runs/camera-01
just liveboot-prepare \
  --fixture "$fixture" --kernel-bundle "$candidate/bundle.json" \
  --device-serial "${TEST_DEVICE_SERIAL:?set the test phone serial}" --run-dir "$run"

# Put the verified test phone in fastboot, then:
just liveboot-boot --run-dir "$run" \
  --uart "${TEST_DEVICE_UART:?set the test phone UART path}"
```
...
just liveboot-prepare \
  --fixture "$fixture" --kernel-bundle "$candidate/bundle.json" \
  --root-mode ram --device-serial "${TEST_DEVICE_SERIAL:?set the test phone serial}" \
...
| --- | --- |
| Known `.8` baseline | 64.70 seconds |
| Exported local `.11` candidate with an explicit kernel bundle | 65.05 seconds |
...
Generated fixtures, raw run outcomes and producer evidence are retained locally
under `out/liveboot/`; they are not published because they include device data.
The table above records the concise validation result. RAM mode remains
