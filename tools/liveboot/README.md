...
For `boot`, put the target in fastboot with host USB permissions configured and keep the process running until the guest shuts down. At this pinned revision, runtime smoo discovery matches interfaces rather than serials and keeps retrying the first smoo-class interface it finds, so connect only one smoo target: a second smoo gadget on the host (for example a DragonBoard liveboot) makes `boot` log `claim usb interface: Resource busy` forever and the guest's initrd gives up waiting for its export. The root is served through gibblox, which keeps a persistent block cache under `~/.cache/gibblox/<id>.bin` keyed by the root file's path and size. After editing a root image in place, delete that entry (or copy the root to a new path and bundle again); otherwise blocks that earlier boots pulled into the cache are served stale, and the guest runs a mix of old and new file contents. Sargo and DB410c reached visible greeters and passed disposable-write checks; reconnects, GPU stability and enforcing SELinux remain unvalidated.
